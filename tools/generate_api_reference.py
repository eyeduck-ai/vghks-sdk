...
PURPOSES = {
    ...
    "prq.visit_cases": "以病歷號或 national_id（二擇一）查就診清單；保留同病人舊病歷號的原始就診連結與查詢號，含日期、類別、科別、醫師。參照 [VISITS](VISITS.md) 篩選並串接。",
    "prq.case_detail": "單次就診的頁籤、連結與原始頁面。",
    "prq.soap": "該次門診的 S／O／A+P、診斷碼、列印醫囑／藥囑摘要及明示的慢性處方服藥期限；保留原文與解析提示，見 [SOAP](SOAP.md)。僅支援 O 類別。",
    "prq.numeric": "該次就診的數值表格；保留雙層表頭、可對齊的欄位路徑及原始數值，見 [NUMERIC_REPORTS](NUMERIC_REPORTS.md)。",
    ...
    "prq.numeric_history": "指定期間的數值結果；與單次就診查詢分開，欄位對齊限制見 [NUMERIC_REPORTS](NUMERIC_REPORTS.md)。",
