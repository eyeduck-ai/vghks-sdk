...
- SDK 原始碼：0.20.4；0.20.1 的院內回傳見下文，0.20.2 修正短單位表頭，0.20.3 依使用者確認的舊病歷號情境保留原始就診號碼。0.20.4 依眼科表格實際回傳，將已能明確對齊的過大 `colspan` 列為測試警示；原始數值及欄位路徑不變。
...
- 本機：0.20.4 的 535 項一般測試完成（523 通過、12 個私有資料測試略過）；啟用本機私有 HAR／回傳只讀回歸後，535 項全部通過。API 目錄與公開內容檢查通過，wheel／sdist 已建置並檢查包內容，wheel 亦於獨立目錄安裝驗證 import／CLI。SOAP 專項曾另啟用私有 HAR 回歸，當時 21 項通過。先前從實際 ZIP 的原始 SOAP 離線重解析確認漏列藥囑補回，並取得一筆明示的慢性處方服藥期限；其他三筆藥囑及各筆 S／O／A+P 不變。
