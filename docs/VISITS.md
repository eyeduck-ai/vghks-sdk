...
**身分證路徑尚未完成內網實測**。目前證據是錄製的前端分支及回應結構，另有合成請求順序、身分核對及重新登入測試。無法辨識查詢成功頁或身分不符時會明確報錯，不會沿用上一位病人的 Session 資料。
...
後續驗證可對同一病人分別使用病歷號與身分證取清單，比對 VisitCase.identity，再選一筆門診查 SOAP／醫囑。只需驗證新增分支，已有成功的其他模組不必為此重測。
