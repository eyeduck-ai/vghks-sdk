...
# 或安裝已建置 wheel
python -m pip install vghks_sdk-0.18.1-py3-none-any.whl
```
...
```sh
python -m pip download --only-binary=:all: --dest wheelhouse vghks_sdk-0.18.1-py3-none-any.whl
python -m pip install --no-index --find-links wheelhouse vghks-sdk==0.18.1
```
