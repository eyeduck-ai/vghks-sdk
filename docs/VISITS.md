...
**身分證路徑已完成 0.17.2 內網實測**：先從基本資料取得身分證，查詢後核對病人，與病歷號清單及醫師／科別欄位比對一致，並從身分證清單選定門診成功取得 SOAP／醫囑。本次證據是一名病人的非空清單與一筆門診抽樣；醫師卡號仍缺來源樣本。無法辨識查詢成功頁或身分不符時仍明確報錯，不會沿用上一位病人的 Session 資料。
